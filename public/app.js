...
window.onload = async () => {
    ...
    if (Array.isArray(window.serverAdd)) {
        ...
        util.pullJSON("gamemodeData").then((json) => {
            document.getElementById("serverName").innerHTML = `<h4 class="nopadding">Finland - ${json.gameMode} | ${json.players} Players</h4>`;
        });
    }
    // Save forms
...
function drawMinimapAndDebug(spacing, alcoveSize) {
    ...
    // Text
    if (global.showDebug) {
        drawText("Doblons.io 1.0", x + len, y - 50 - 5 * 14 - 2, 15, "#B6E57C", "right");
        ...
        drawText("Doblons.io 1.0", x + len, y - 50 - 2 * 14 - 2, 15, "#B6E57C", "right");
        drawText((100 * gui.fps).toFixed(2) + "% : " + global.metrics.rendertime + " FPS" + " / " + global.metrics.updatetime + "Hz", x + len, y - 50 - 1 * 14, 10, global.metrics.rendertime > 10 ? color.guiwhite : color.orange, "right");
        drawText(global.metrics.latency + " ms : " + "fi-helsinki", x + len, y - 50, 10, color.guiwhite, "right");
    }
...
function animloop() {
    ...
    var ratio = settings.graphical.screenshotMode ? 1 : util.getRatio();
